...
atendente. Para **encerrar atendimento humano**, o frontend usa exclusivamente
`actions/return-to-bot`. `actions/close` é um comando separado, exposto como
**Encerrar**, que coloca a conversa canônica no estado temporário `closed`; o
próximo contato reabre a mesma conversa e preserva o histórico. A rota
`close-after-rejection` existe somente como alias legado e não é emitida pelo
painel.
...
O navegador nunca chama Pelias ou OpenRouteService diretamente e não calcula distância,
